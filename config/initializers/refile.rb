...
if Rails.env.production? && Admin::Settings['REFILE_USE_S3'] == 'true'
  ...
  Refile.backends['s3_project_attach_backend'] = Refile::S3.new(prefix: "store_project_attach", max_size: Admin::Settings['REFILE_S3_PROJECT_ATTACH_MAX_FILESIZE'].to_i.megabytes, **aws)
  Refile.backends['s3_project_attach_cache'] = Refile::S3.new(prefix: "cache_project_attach", max_size: Admin::Settings['REFILE_S3_PROJECT_ATTACH_MAX_FILESIZE'].to_i.megabytes, **aws)
  ...
  Refile.backends['s3_project_attach_backend'] = Refile::Backend::FileSystem.new(Rails.root.join('uploads/s3_project_attach/store').to_s, max_size: Admin::Settings['REFILE_S3_PROJECT_ATTACH_MAX_FILESIZE'].to_i.megabytes)
  Refile.backends['s3_project_attach_cache'] = Refile::Backend::FileSystem.new(Rails.root.join('uploads/s3_project_attach/cache').to_s, max_size: Admin::Settings['REFILE_S3_PROJECT_ATTACH_MAX_FILESIZE'].to_i.megabytes)
end
